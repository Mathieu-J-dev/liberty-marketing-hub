    ...
    <div className="fixed bottom-6 right-6 z-50">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button size="lg" className="shadow-lg bg-liberty-600 hover:bg-liberty-700 text-white border-0 rounded-full">
            <MessageCircle className="mr-2 h-5 w-5" /> Assistant IA
          </Button>
        </PopoverTrigger>
        <PopoverContent 
          className="w-96 p-4 bg-card text-card-foreground border-liberty-200 dark:border-liberty-800 shadow-xl" 
          side="top" 
          align="end"
          sideOffset={10}
        >
